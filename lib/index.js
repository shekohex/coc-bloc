...
var require_range = __commonJS((exports2, module2) => {
  ...
  var replaceTilde = (comp, options) => {
    ...
    return comp.replace(r, (_5, M, m, p, pr) => {
      debug("tilde", comp, _5, M, m, p, pr);
      let ret;
      if (isX(M)) {
        ret = "";
      } else if (isX(m)) {
...
function getPubspecPath() {
  if (import_coc.workspace.workspaceFolders && import_coc.workspace.workspaceFolders.length > 0) {
    return path.join(`${import_coc.workspace.workspaceFolders[0].uri}`, "pubspec.yaml");
  }
}
...
async function getPubspec() {
  ...
  if (pubspecPath) {
    try {
      const content = await import_coc2.workspace.readFile(import_coc2.Uri.file(pubspecPath).fsPath);
      return load$1(content.toString());
    } catch (_5) {
    }
  }
}
...
async function hasDependency(dependency) {
  ...
  const dependencies = import_lodash.default.get(pubspec, "dependencies", {});
  return import_lodash.default.has(dependencies, dependency);
}

// src/utils/get-template-setting.ts
var import_coc3 = __toModule(require("coc.nvim"));
var TemplateSetting;
(function(TemplateSetting2) {
  TemplateSetting2[TemplateSetting2["Auto"] = 0] = "Auto";
  TemplateSetting2[TemplateSetting2["Equatable"] = 1] = "Equatable";
  TemplateSetting2[TemplateSetting2["Freezed"] = 2] = "Freezed";
  TemplateSetting2[TemplateSetting2["Simple"] = 3] = "Simple";
})(TemplateSetting || (TemplateSetting = {}));
var TemplateType;
(function(TemplateType3) {
  TemplateType3[TemplateType3["Bloc"] = 0] = "Bloc";
  TemplateType3[TemplateType3["Cubit"] = 1] = "Cubit";
})(TemplateType || (TemplateType = {}));
function getTemplateSetting(type2) {
  let config;
  switch (type2) {
    case 0:
      config = import_coc3.workspace.getConfiguration("bloc").get("newBlocTemplate");
      break;
    case 1:
      config = import_coc3.workspace.getConfiguration("bloc").get("newCubitTemplate");
      break;
    default:
      return 0;
  }
...
var BlocType;
(function(BlocType4) {
  BlocType4[BlocType4["Simple"] = 0] = "Simple";
  BlocType4[BlocType4["Equatable"] = 1] = "Equatable";
  BlocType4[BlocType4["Freezed"] = 2] = "Freezed";
})(BlocType || (BlocType = {}));
async function getBlocType(type2) {
  const setting = getTemplateSetting(type2);
  switch (setting) {
...
function updatePubspecDependency(dependency) {
  if (import_coc4.workspace.workspaceFolders && import_coc4.workspace.workspaceFolders.length > 0) {
    const pubspecPath = getPubspecPath();
    if (pubspecPath) {
      try {
...
async function analyzeDependencies() {
  ...
  const pubspec = await getPubspec();
  const dependencies = import_lodash2.default.get(pubspec, "dependencies", {});
  const devDependencies = import_lodash2.default.get(pubspec, "dev_dependencies", {});
...
function checkForUpgrades(dependenciesToAnalyze, dependencies) {
  for (let i = 0; i < dependenciesToAnalyze.length; i++) {
    ...
    if (import_lodash2.default.has(dependencies, dependency.name)) {
      const dependencyVersion = import_lodash2.default.get(dependencies, dependency.name, "latest");
      ...
      if (typeof dependencyVersion !== "string")
        continue;
      const minVersion2 = import_lodash2.default.get(semver.minVersion(dependencyVersion), "version", "0.0.0");
      if (!semver.satisfies(minVersion2, dependency.version)) {
        import_coc5.window.showWarningMessage(`This workspace contains an unsupported version of ${dependency.name}. Please update to ${dependency.version}.`, ...dependency.actions.map((action) => action.name).concat("Update")).then((invokedAction) => {
          ...
          if (!import_lodash2.default.isNil(action)) {
            action.callback();
          }
        });
      }
...
// src/templates/bloc-event.template.ts
function getBlocEventTemplate(blocName, type2) {
  switch (type2) {
...
function getFreezedBlocEvent(blocName) {
  const pascalCaseBlocName = pascalCase(blocName.toLowerCase()) + "Event";
  const snakeCaseBlocName = snakeCase(blocName.toLowerCase());
  return `part of '${snakeCaseBlocName}_bloc.dart';
...
function getBlocStateTemplate(blocName, type2) {
  switch (type2) {
    case BlocType.Freezed:
...
function getFreezedBlocStateTemplate(blocName) {
  const pascalCaseBlocName = pascalCase(blocName.toLowerCase()) + "State";
  const snakeCaseBlocName = snakeCase(blocName.toLowerCase());
  return `part of '${snakeCaseBlocName}_bloc.dart';
...
function getBlocTemplate(blocName, type2) {
  switch (type2) {
    case BlocType.Freezed:
...
function getCubitStateTemplate(cubitName, type2) {
  switch (type2) {
    case BlocType.Freezed:
...
function getCubitTemplate(cubitName, type2) {
  switch (type2) {
    case BlocType.Freezed:
...
var newBloc = async (uri) => {
  ...
  if (import_lodash3.default.isNil(blocName) || blocName.trim() === "") {
    import_coc6.window.showErrorMessage("The bloc name must not be empty");
    return;
  }
  let targetDirectory;
  if (import_lodash3.default.isNil(import_lodash3.default.get(uri, "fsPath")) || !import_fs.lstatSync(uri.fsPath).isDirectory()) {
    const d = await promptForTargetDirectory();
    if (import_lodash3.default.isNil(d)) {
      import_coc6.window.showErrorMessage("Please type a valid directory");
      return;
    }
    targetDirectory = import_path.default.join("lib", d);
  } else {
    targetDirectory = import_path.default.join("lib", uri.fsPath);
  }
  const blocType = await getBlocType(TemplateType.Bloc);
  const pascalCaseBlocName = pascalCase(blocName.toLowerCase());
  try {
    await generateBlocCode(blocName, targetDirectory, blocType);
    import_coc6.window.showInformationMessage(`Successfully Generated ${pascalCaseBlocName} Bloc`);
  } catch (error) {
    import_coc6.window.showErrorMessage(`Error:
        ${error instanceof Error ? error.message : JSON.stringify(error)}`);
  }
};
function promptForBlocName() {
  return import_coc6.workspace.callAsync("input", ["Bloc name (example: conter): "]);
}
async function promptForTargetDirectory() {
  return import_coc6.workspace.callAsync("input", [
    "Folder path to create the bloc in (example: blocs): "
  ]);
}
async function generateBlocCode(blocName, targetDirectory, type2) {
  const blocDirectoryPath = `${targetDirectory}/bloc`;
  if (!import_fs.existsSync(blocDirectoryPath)) {
    await createDirectory(blocDirectoryPath);
  }
  await Promise.all([
    createBlocEventTemplate(blocName, targetDirectory, type2),
    createBlocStateTemplate(blocName, targetDirectory, type2),
    createBlocTemplate(blocName, targetDirectory, type2)
  ]);
}
...
function createBlocEventTemplate(blocName, targetDirectory, type2) {
  const snakeCaseBlocName = snakeCase(blocName.toLowerCase());
  ...
  return new Promise(async (resolve, reject) => {
    import_fs.writeFile(targetPath, getBlocEventTemplate(blocName, type2), "utf8", (error) => {
      if (error) {
        reject(error);
        return;
      }
...
function createBlocStateTemplate(blocName, targetDirectory, type2) {
  const snakeCaseBlocName = snakeCase(blocName.toLowerCase());
  ...
  return new Promise(async (resolve, reject) => {
    import_fs.writeFile(targetPath, getBlocStateTemplate(blocName, type2), "utf8", (error) => {
      if (error) {
        reject(error);
        return;
      }
...
function createBlocTemplate(blocName, targetDirectory, type2) {
  const snakeCaseBlocName = snakeCase(blocName.toLowerCase());
  ...
  return new Promise(async (resolve, reject) => {
    import_fs.writeFile(targetPath, getBlocTemplate(blocName, type2), "utf8", (error) => {
      if (error) {
        reject(error);
        return;
      }
...
var mkdirp2 = __toModule(require_mkdirp());
var import_path2 = __toModule(require("path"));
var newCubit = async (uri) => {
  const cubitName = await promptForCubitName();
  if (import_lodash4.default.isNil(cubitName) || cubitName.trim() === "") {
    import_coc7.window.showErrorMessage("The cubit name must not be empty");
    return;
  }
  let targetDirectory;
  if (import_lodash4.default.isNil(import_lodash4.default.get(uri, "fsPath")) || !import_fs2.lstatSync(uri.fsPath).isDirectory()) {
    const d = await promptForTargetDirectory2();
    if (import_lodash4.default.isNil(d)) {
      import_coc7.window.showErrorMessage("Please type a valid directory");
      return;
    }
    targetDirectory = import_path2.default.join("lib", d);
  } else {
    targetDirectory = import_path2.default.join("lib", uri.fsPath);
  }
  const blocType = await getBlocType(TemplateType.Cubit);
  const pascalCaseCubitName = pascalCase(cubitName.toLowerCase());
  try {
    await generateCubitCode(cubitName, targetDirectory, blocType);
    import_coc7.window.showInformationMessage(`Successfully Generated ${pascalCaseCubitName} Cubit`);
  } catch (error) {
    import_coc7.window.showErrorMessage(`Error:
        ${error instanceof Error ? error.message : JSON.stringify(error)}`);
  }
};
function promptForCubitName() {
  return import_coc7.workspace.callAsync("input", ["Cubit name (example: conter): "]);
}
async function promptForTargetDirectory2() {
  return import_coc7.workspace.callAsync("input", [
    "Folder path to create the cubit in (example: blocs): "
  ]);
}
async function generateCubitCode(cubitName, targetDirectory, type2) {
  const cubitDirectoryPath = `${targetDirectory}/cubit`;
  if (!import_fs2.existsSync(cubitDirectoryPath)) {
    await createDirectory2(cubitDirectoryPath);
  }
  await Promise.all([
    createCubitStateTemplate(cubitName, targetDirectory, type2),
    createCubitTemplate(cubitName, targetDirectory, type2)
  ]);
}
...
function createCubitStateTemplate(cubitName, targetDirectory, type2) {
  const snakeCaseCubitName = snakeCase(cubitName.toLowerCase());
  const targetPath = `${targetDirectory}/cubit/${snakeCaseCubitName}_state.dart`;
  if (import_fs2.existsSync(targetPath)) {
    throw Error(`${snakeCaseCubitName}_state.dart already exists`);
  }
  return new Promise(async (resolve, reject) => {
    import_fs2.writeFile(targetPath, getCubitStateTemplate(cubitName, type2), "utf8", (error) => {
      if (error) {
        reject(error);
        return;
      }
...
function createCubitTemplate(cubitName, targetDirectory, type2) {
  const snakeCaseCubitName = snakeCase(cubitName.toLowerCase());
  ...
  return new Promise(async (resolve, reject) => {
    import_fs2.writeFile(targetPath, getCubitTemplate(cubitName, type2), "utf8", (error) => {
      if (error) {
        reject(error);
        return;
      }
...
async function activate(context) {
  if (import_coc8.workspace.getConfiguration("bloc").get("checkForUpdates")) {
    analyzeDependencies();
  }
  context.subscriptions.push(import_coc8.commands.registerCommand("coc-bloc.new-bloc", newBloc), import_coc8.commands.registerCommand("coc-bloc.new-cubit", newCubit));
}
